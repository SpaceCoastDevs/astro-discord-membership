...
export async function getGuildMembership(accessToken: string, guildId: string): Promise<DiscordGuildMembership | null> {
...
export async function isGuildMember(accessToken: string, guildId: string): Promise<boolean> {
  return (await getGuildMembership(accessToken, guildId)) !== null;
}
