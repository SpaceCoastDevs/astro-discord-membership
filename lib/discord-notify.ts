import { communityName, siteUrl } from 'astro-discord-membership:config';
...
export async function notifyNewMember(member: MemberNotifyPayload): Promise<void> {
  ...
  const profileUrl = `${siteUrl}/members/${member.discordId}`;
    ...
    embeds: [
      {
        ...
        title: `New member profile on ${communityName}`,
        url: profileUrl,
        description: bioExcerpt ?? undefined,
        color: 0x5865f2, // Discord blurple
        fields,
        footer: { text: `${siteUrl}/members` },
      },
    ],
  };
